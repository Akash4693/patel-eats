...
var CheckoutConfirmPage_1 = require("./CheckoutConfirmPage");
var useCartStore_1 = require("@/store/useCartStore");
var Cart = function () {
    var _a = (0, react_1.useState)(false), open = _a[0], setOpen = _a[1];
    var _b = (0, useCartStore_1.useCartStore)(), cart = _b.cart, decrementQuantity = _b.decrementQuantity, incrementQuantity = _b.incrementQuantity;
    var totalAmount = cart.reduce(function (acc, ele) {
        return acc + ele.price * ele.quantity;
    }, 0);
    return (<div className="flex flex-col max-w-7xl mx-auto my-10">
      <div className="flex justify-end">
        <button_1.Button variant="link">Clear All</button_1.Button>
      </div>
      <table_1.Table>
        <table_1.TableHeader>
          <table_1.TableRow>
        ...
        <table_1.TableBody>
          {cart.map(function (item) { return (<table_1.TableRow>
              <table_1.TableCell>
                <avatar_1.Avatar>
                  <avatar_1.AvatarImage src={item.image} alt=""/>
              ...
              <table_1.TableCell>
                <div className="w-fit flex items-center rounded-full border border-gray-100 dark:border-gray-800 shadow-md">
                  <button_1.Button onClick={function () { return decrementQuantity(item._id); }} size={"icon"} variant={"outline"} className="rounded-full bg-gray-200">
                    <lucide_react_1.Minus />
                  </button_1.Button>
                  <button_1.Button size={"icon"} className="font-bold border-none" disabled variant={"outline"}>
                    {item.quantity}
                  </button_1.Button>
                  <button_1.Button onClick={function () { return incrementQuantity(item._id); }} size={"icon"} className="rounded-full bg-orange hover:bg-hoverOrange" variant={"outline"}>
                    <lucide_react_1.Plus />
                  </button_1.Button>
                </div>
              </table_1.TableCell>
              ...
              <table_1.TableCell className="text-right">
                <button_1.Button size={"sm"} className="bg-orange hover:bg-hoverOrange">
                  Remove
                </button_1.Button>
              </table_1.TableCell>
            </table_1.TableRow>); })}
      ...
      <div className="flex justify-end my-5">
        <button_1.Button onClick={function () { return setOpen(true); }} className="bg-orange hover:bg-hoverOrange">
          Proceed To Checkout
        </button_1.Button>
      </div>
